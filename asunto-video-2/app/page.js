...
export default function Home() {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
      ...
      try {
        const res = await fetch(
          `/api/status?requestId=${encodeURIComponent(requestId)}&apiKey=${encodeURIComponent(
            apiKey
  ...
  const mergeClips = async (orderedVideoUrls) => {
    ...
    try {
      const res = await fetch("/api/merge", {
        ...
        body: JSON.stringify({ videoUrls: orderedVideoUrls, apiKey }),
      });
      const data = await res.json();
      ...
      const videoUrl = await pollForResult(data.requestId, data.model);
      if (!videoUrl) {
        setMergeState("error");
        return;
      }
  ...
  const generateAll = async () => {
    ...
    if (!apiKey.trim()) {
      alert("Liitä ensin fal.ai-API-avain yläpalkin kenttään.");
      return;
    }
    setIsRunning(true);
    setMergeState("idle");
    ...
    await Promise.all(
      plates.map(async (plate) => {
        updatePlate(plate.id, { status: "scanning" });
        try {
          const res = await fetch("/api/generate", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ image: plate.preview, motion: plate.motion, apiKey }),
          });
          const data = await res.json();
          if (!res.ok || !data.requestId) {
            updatePlate(plate.id, { status: "error" });
            return;
          }
          const videoUrl = await pollForResult(data.requestId, data.model);
          if (!videoUrl) {
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={styles.keyBar}>
        <label className={styles.keyLabel} htmlFor="apiKey">
          FAL API-AVAIN
        </label>
        <input
          id="apiKey"
      ...
      {plates.length > 0 && (
        <section className={styles.plates}>
          {plates.map((plate, index) => (
            <div key={plate.id} className={styles.plate}>
      ...
      {mergeState === "error" && (
        <div className={`${styles.mergeBox} ${styles.mergeBoxError}`}>
          Klippien yhdistäminen epäonnistui. Yksittäiset klipit ovat silti
          käytettävissä yllä olevista linkeistä.
        </div>
      )}

...
export async function POST(request) {
  try {
    if (!process.env.FAL_KEY) {
      return NextResponse.json({ error: 'FAL_KEY puuttuu palvelimen ympäristömuuttujista.' }, { status: 500 });
    }
    ...
    for (const image of images) {
      ...
      const file = new File([blob], image.name || 'room-image.jpg', { type: image.type });
